...
(define (collect-garbage some-roots more-roots)
  ...
  (for ([i (in-range (compact-live! 0 0) (heap-size))])
    (heap-set! i 'free)))
...
(define (compact-live! free-ptr live-ptr)
  (define next-live (find-next-live live-ptr))
  (cond
    [(and next-live (< free-ptr (heap-size)))
     (case (heap-ref next-live)
       [(pair) (heap-copy! free-ptr next-live 3)
               (compact-live! (+ 3 free-ptr) (+ 3 next-live))]
       [(flat) (heap-copy! free-ptr next-live 2)
               (compact-live! (+ 2 free-ptr) (+ 2 next-live))]
       [(proc) (define proc-size (+ next-live 3 (heap-ref (+ next-live 2))))
               (heap-copy! free-ptr next-live proc-size)
               (compact-live! (+ proc-size free-ptr) (+ proc-size next-live))]
       [(white-pair white-flat white-proc) (void)]
       [else (error 'compact-live! "unknown tag ~s" (heap-ref next-live))])]
    [else free-ptr]))

(define (find-next-live i)
  (displayln i)
  (and (< i (heap-size))
       (case (heap-ref i)
         [(white-pair) (find-next-live (+ i 3))]
         [(white-flat) (find-next-live (+ i 2))]
         [(white-proc) (find-next-live (+ i 3 (heap-ref (+ i 2))))]
         [(pair flat proc) i]
         [else (find-next-live (+ i 1))])))

(define (heap-copy! next-free next-live size)
  (for ([i (in-range size)])
    (heap-set! (+ i next-free) (heap-ref (+ i next-live)))))
