...
(let/ec out
  (let loop ([matched 0] [out out] [in '(#\  #\a #\b #\c)])
    (cond
      [(null? in) (out null)]
      [(or (zero? matched) (= 2 matched))
       (cons (car in) 
             (let/ec out 
               (loop (add1 matched) out (cdr in))))]
      [else 
       (print (car in))
       (cons (car in) 
             (loop (add1 matched) out (cdr in)))])))

(let loop ([matched 0] [in '(#\  #\a #\b #\c)])
  (cond
    [(null? in) null]
    [(or (zero? matched) (= 2 matched)) 
     (cons (car in) 
           (loop (add1 matched) (cdr in)))]
    [else 
     (print (car in))
     (let ([rest (loop (add1 matched) (cdr in))])
       (if (null? rest)
           null
           (cons (car in) rest)))]))
