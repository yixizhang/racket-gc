...
(define (gc:vector-ref loc number)
  ...
  (cond
    [(< number (gc:vector-length loc)) (heap-ref/bm (+ loc 2 number))]
...
(define (gc:vector-set! loc number thing)
  ...
  (cond 
    [(< number (gc:vector-length loc)) 
     (begin
       (heap-set!/bm (+ loc 2 number) thing)
