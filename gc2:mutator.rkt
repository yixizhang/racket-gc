...
(provide else require provide #%top
         ...
         (rename-out
          ...
          (mutator-cdr cdr)
          ;; ToDo
          (mutator-append append)
          ;; ToDo
          (mutator-letrec letrec)
          ;; ToDo
          (mutator-make-parameter make-parameter)
          ;; ToDo
          (mutator-current-input-port current-input-port)
          ;; ToDo
          (mutator-compose compose)))
...
(define-syntax-rule (mutator-if test true false)
  (if (syntax-parameterize ([mutator-tail-call? #f])
      ...
      (->address false)))
;; ToDo
(define-syntax-rule (mutator-unless test true)
  (void))
...
;; ToDo
(define-syntax-rule (mutator-foldr proc init lst ...)
  (void))

;; ToDo : (compose proc ...) -> procedure?
;; returns a procedure that composes the given functions, applying the last proc first and the first proc last.
(define-syntax-rule (mutator-compose proc ...)
  (void))

;; ToDo 
;; like member, but finds an element using the predicate proc.
(define-syntax-rule (mutator-memf proc lst)
  (void))

;; ToDo
;; like member, but finds an element using eq?
(define-syntax-rule (mutator-memq v lst)
  (void))
...
;; ToDo
(define (mutator-null? thing)
  (eq? thing '()))

;; ToDo
(define-syntax-rule (mutator-car lst)
  (void))

;; ToDo
(define-syntax-rule (mutator-cdr lst)
  (void))

;; ToDo
(define-syntax-rule (mutator-append lst ...)
  (void))

;; ToDo
(define-syntax-rule (mutator-letrec ([id val-expr] ...) body ...)
  (void))

;; ToDo
;; by definition : (make-parameter v [guard]) -> procedure?
;; returns a new parameter procedure.
(define-syntax-rule (mutator-make-parameter v)
  (void))

;; ToDo
(define-syntax-rule (mutator-current-input-port)
  (void))
