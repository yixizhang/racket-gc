...
(provide/contract (add-extra-roots! ((listof root?) . -> . void?)))
(define (add-extra-roots! roots)
  (set! extra-roots (append roots extra-roots)))
...
(provide/contract (remove-extra-roots! ((listof root?) . -> . void?)))
(define (remove-extra-roots! roots)
  (set! extra-roots
        (let loop ([l roots] [result extra-roots])
          (if (null? l)
              result
