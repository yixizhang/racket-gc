...
(define (gc:cons hd tl)
  ...
  (when (or (2nd-gen? head)
            (2nd-gen? tail))
    (table/alloc ptr empty))
  ptr)
...
(define (gc:closure code-ptr free-vars)
  ...
  (when (for/or ([v (in-vector updated-free-vars)])
          (2nd-gen? v))
    (table/alloc next empty))
  next)
...
(define (gc:alloc-struct-instance s fields-value)
  ...
  (when (or (2nd-gen? ss)
            (for/or ([v (in-vector fields-value)])
              (2nd-gen? v)))
    (table/alloc next empty))
  next)
...
;; for reference from big subheap, alloc (loc, loc)
;; for reference from small subheap, alloc (loc, empty)
...
(define (forward/pointers)
  (local [(define (forward/1-2 loc)
            (case (heap-ref/bm loc)
              [(cons)
               (define hd (heap-ref/bm (+ loc 1)))
               (define tl (heap-ref/bm (+ loc 2)))
               (when (2nd-gen? hd)
                 (trace/roots-iff-white hd))
               (when (2nd-gen? tl)
                 (trace/roots-iff-white tl))]
              [(vector)
               (define length (heap-ref/bm (+ loc 1)))
               (for ([i (in-range length)])
                 (define target (heap-ref/bm (+ loc 2 i)))
                 (when (2nd-gen? target)
                   (trace/roots-iff-white target)))]
              [(struct)
               (define parent (heap-ref/bm (+ loc 2)))
               (when (2nd-gen? parent)
                 (trace/roots-iff-white parent))]
              [(struct-instance)
               (define v-count (heap-ref/bm (+ (heap-ref/bm (+ loc 1)) 3)))
               (define s (heap-ref/bm (+ loc 1)))
               (when (2nd-gen? s)
                 (trace/roots-iff-white s))
          ...
          (define (loop start)
            (cond
              [(or (= start 2nd-gen-start)
                   (equal? 'free (heap-ref/bm start)))
               (void)]
              [else
                (define from (heap-ref/bm start))
                (cond
                  [(and (1st-gen? from)
                        (equal? 'frwd (heap-ref/bm from)))
                   (forward/1-2 from)]
                  [(2nd-gen? from)
                   (define to (heap-ref/bm (+ start 1)))
                   (when (not (equal? 'frwd (heap-ref/bm to)))
                     (heap-set!/bm from (forward/loc to)))])
                ;; forward the next pair
                (loop (+ 2 start))]))]
         (loop (add1 table-start-word))
         (loop (add1 table-start-word)))
  (for ([i (in-range (add1 table-start-word) 2nd-gen-start 2)]
        #:break (equal? 'free (heap-ref/bm i)))
       (heap-set!/bm i 'free)
       (heap-set!/bm (add1 i) 'free))
