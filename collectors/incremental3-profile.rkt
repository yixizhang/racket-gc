...
(define (init-allocator)
  ...
  (for ([i (in-range 0 (heap-size))])
    (heap-set! i 'free))
  (set! 1st-gen-size (round (* (heap-size) 1/11)))
...
;; batch-up incremental tracing config
(define BATCH-UP-COUNT 0)
(define BATCH-UP-LIMIT 7) ;; batch-up para = 4
...
(define (collect-garbage some-roots more-roots)
  ...
  ;; batch-up incremental tracing
  (cond [(= BATCH-UP-COUNT BATCH-UP-LIMIT)
         (set! BATCH-UP-COUNT 0)
         (traverse/incre-mark (next/cont))]
        [else (set! BATCH-UP-COUNT (add1 BATCH-UP-COUNT))])
