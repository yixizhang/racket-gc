...
(define (gc:cons hd tl)
  ...
  (define head (track/loc (read-root hd)))
  (define tail (track/loc (read-root tl)))
  (heap-set! ptr 'pair)
  (heap-set! (+ ptr 1) head)
  (heap-set! (+ ptr 2) tail)
  ptr)
...
;; vector related
(define (gc:vector length value)
  (define next (alloc (+ length 2) value #f))
  (define l (track/loc (read-root value)))
...
(define (gc:alloc-struct name parent fields-count)
  ...
  (define p (and parent (track/loc (read-root parent))))
...
(define (gc:struct? loc)
  (case (heap-ref loc)
    [(struct white-struct grey-struct) #t]
    [else #f]))
...
(define (gc:alloc-struct-instance s fields-value)
  (define fv-count (length fields-value))
  (define next (alloc (+ fv-count 2)
                      s
                      fields-value))
  (define ss (track/loc (read-root s)))
  (define fields
    (for/vector ([v (in-list fields-value)])
      (track/loc (read-root v))))
  (heap-set! (+ next 1) ss)
  (for ([x (in-range 0 fv-count)])
    (heap-set! (+ next 2 x)
...
;; loc number -> bool
(define (enough-space-to-space? start size)
  (define-values (begin end) (to-space))
  (< (+ start size) end))

;; number -> loc
(define (switch/sweep-tospace number)
  ;; switch to-space and sweep to-space
  ;; new tospace is old fromspace
